...
*/
  ...
  /* below works but changed using above
  jwt.verify(token, "well well well", function(error, decoded) {
    if (decoded) {
      let username = decoded.username;
      console.log("decoded:", username);
      ...
      db.one(sql, [username])
        ...
        .catch(error => {
          res.status(400).json({
            message:
              "User does not have access to specific route(s) - ie add/modify books",
            error
          });
        });
*/
...
// next();
// });  uncomment if reverting back to app.use
// }  for function
